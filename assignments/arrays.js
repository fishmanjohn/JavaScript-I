...
for (i = 0;i < carInventory.length; i++){
  if (carInventory[i].id === iD){
    return `car 33 is a ${carInventory[i].caryear}  ${carInventory[i].car_make}  ${carInventory[i].car_model}`
    }
  }
}
...
const autoAnyos = carYears();

const preY2k = () => {
 let oldCars = []
for(let i = 0; i<autoAnyos.length; i++){
  if (autoAnyos[i] < 2000 ){
    oldCars.push(autoAnyos[i])
  } 
}
